  ...
  try {
    ...
    if (progressSheet) {
      ...
      for (let i = 1; i < progressData.length; i++) {
        ...
        const pVideoTime = progressData[i][3] || 0; // Column D = VideoTime
        const pScore = progressData[i][5] || 0; // Column F = Score
...
// ==========================================
// 10. ADVANCED MANAGEMENT FEATURES (NEW)
// ==========================================
...
function addStudentToSheet(email, name) {
  ...
  try {
    ...
    // Default values
    const timestamp = new Date().toLocaleString('vi-VN', {timeZone: 'Asia/Ho_Chi_Minh'});
    const defaultPass = hashPassword('123456'); // Default password
    const rowData = [
      emailLower, 
      name, 
      '', // Phone
      defaultPass, 
      'Approve', // Auto approve since Admin added
      timestamp,
      '' // Allowed Courses
    ];
...
function toggleAutoReminder(isActive) {
  try {
    const triggers = ScriptApp.getProjectTriggers();
    const handlerName = 'runAutoReminder';
    ...
    if (isActive) {
      ...
      ScriptApp.newTrigger(handlerName)
        ...
        .atHour(9)
        .create();
      return { success: true, message: 'Đã BẬT tự động nhắc nhở (9:00 sáng hàng ngày).' };
    } else {
      return { success: true, message: 'Đã TẮT tự động nhắc nhở.' };
    }
    
  } catch (e) {
    return { success: false, message: 'Lỗi Trigger (Bạn cần cấp quyền): ' + e.toString() };
  }
}

function getAutoReminderStatus() {
  const triggers = ScriptApp.getProjectTriggers();
  const handlerName = 'runAutoReminder';
  const exists = triggers.some(t => t.getHandlerFunction() === handlerName);
  return { success: true, isActive: exists };
}

// Hàm chạy bởi Trigger (Background Job)
function runAutoReminder() {
  console.log('⏰ Auto Reminder Running...');
  // Logic: Tìm user 'Inactive' > 7 ngày (tính theo LastUpdate trong Sheet Progress)
  // Demo logic: Gửi mail mẫu cho Admin báo cáo
  try {
     const email = Session.getActiveUser().getEmail(); // Email của chủ script (Admin)
     MailApp.sendEmail({
       to: email,
       subject: '[LMS System] Báo cáo Auto Reminder',
       body: 'Hệ thống đã quét và (giả lập) gửi email nhắc nhở cho các học viên không hoạt động.'
     });
  } catch (e) {
...
// (Duplicate getEmailStats removed)
...
function getEmailStats() {
   ...
   campData.forEach((row, i) => {
       ...
       let rowDate = row[1];
       // CUSTOM PARSE for "HH:mm:ss d/M/yyyy" (e.g. "16:14:17 6/2/2026")
       if (typeof rowDate === 'string' && rowDate.includes('/')) {
            try {
                ...
                if(d.length === 3) {
                     ...
                     rowDate = new Date(d[2], parseInt(d[1])-1, d[0]);
                } else {
                     rowDate = new Date(rowDate);
                }
            } catch(e) { rowDate = new Date(rowDate); }
       } else {
